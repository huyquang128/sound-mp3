...
export default function NewSong() {
    ...
    const newArrSong = useMemo(() => {
        const arrResult = listSongsNew.filter(
            (item) => item.countTry === activeArea
        );
        return arrResult;
    }, [activeArea]);
        ...
        <div className=''>
            ...
            <div className="flex items-center gap-2.5 uppercase">
                {areaSong.map((item, index) => (
                    <div
                        onClick={() => setActiveAria(item.value)}
                        key={index}
                        className={`text-white font-normal text-xs px-6 py-1  rounded-full cursor-pointer ${
                            activeArea === item.value ? 'bg-btn-primary' : ''
                        }`}
                    >
                        {item.category}
                    </div>
